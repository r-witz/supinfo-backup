$(document).ready(function () {
    ...
    //Audio Vars
    backAudio = new Audio('marioMainTheme.mp3');
    barAudio = new Audio('smb_kick.wav');
    bonusBirthAudio = new Audio('smb_powerup_appears.wav');
    bonusAudio = new Audio('smb_powerup.wav');
    gameOverAudio = new Audio('smb_gameover.wav');
    gameWinAudio = new Audio("smb_stage_clear.wav");
    ...
    function generateBoard() {
        ...
        for (var currentRow = 0; currentRow < nbrRow; currentRow++) {
            ...
            for (var currentColumn = 0; currentColumn < nbrColumns; currentColumn++) {
                ...
                if (generateCell <= 20) {
                    $("#row" + currentRow).append("<td id='black" + tempID + "'></td>");
                    $("#black" + tempID).css("background-color", "rgba(0, 0, 0, 0)");
                } else {
                    $("#row" + currentRow).append("<td id='break" + tempID + "'></td>");
                    $newBrick = $("#break" + tempID);
                    $newBrick.css({
    ...
    function placePlayingBar() {
        ...
        $pressPlay.css({
            "top": "70%",
            "left": ($gameArea.width() / 2 - $pressPlay.width() / 2)
        });
        $playingBarPosition = {
        ...
        if ($playingBarPosition.Bottom + $playingBarPosition.Height <= y - 2) {
            if (ballsList.length > 1) {
                ...
                if (lives > 1) {
                    ...
                    lives--;
                    refreshLiveCount();
                    $("#death").css("display", "flex");
                    $('.marioDeathGIF').toggleClass('marioDeathGIFAnimated');
                    backAudio.pause();
                    gameOverAudio.play();
                    ballsList[ballNumber].Selector.remove();
                }
    ...
    function brickTouched(nbrBriques) {
        brickAudio = new Audio('smb_fireball.wav');
        brickAudio.play();
        $("#" + bricksList[nbrBriques].ID).css("background", "none");
        refreshPointsCount();
    ...
    /**Les 4 fonction qui suivent sont spécifiques à la version Mario
     * Elles gèrent l'affichage aléatoire ainsi que l'animation des nuages en arrière-plan
     */
    function nuageAnimation(cloudListNumber, left) {
        $currentCloud = $("#" + cloudList[cloudListNumber].ID);
        cloudList[cloudListNumber].Left = parseInt($currentCloud.css("left")) - 1;
        $currentCloud.css("left", cloudList[cloudListNumber].Left);
        if (cloudList[cloudListNumber].Left + parseInt($currentCloud.css("width")) < 0) {
            $currentCloud.remove();
            cloudList.splice(cloudListNumber, 1);
        }
    }

    function cloudInterval() {
        nuageAnimationTimer = setTimeout(function () {
            for (var i = 0; i < cloudList.length; i++) {
                nuageAnimation(i);
            }
            cloudIntervalChangeSpeed();
            clearTimeout(nuageAnimationTimer);
        }, cloudSpeed);
    }

    function cloudIntervalChangeSpeed() {
        nuageAnimationTimerChange = setTimeout(function () {
            for (var i = 0; i < cloudList.length; i++) {
                nuageAnimation(i);
            }
            cloudInterval();
            clearTimeout(nuageAnimationTimerChange);
        }, cloudSpeed);
    }
    ...
    function randomCloud() {
        setInterval(function () {
            if ((Math.floor(Math.random() * 100) + 1) < 80) {
                calculatedTop = Math.floor(Math.random() * ($(Window).height() / 2)) + ($(Window).height() / 10);
                cloudnumber++;
                cloudList.push({
                    "ID": "cloudnumber" + cloudnumber,
                    "Style": (Math.floor(Math.random() * 2) + 1),
                    "Top": calculatedTop,
                    "Left": $(window).width()
                });
                console.log(cloudList[cloudList.length - 1].Left);
                newNuage = new nuageAnimation(cloudList.length - 1);
                $("body").append("<img src='cloud" + cloudList[cloudList.length - 1].Style + ".png' id=" + cloudList[cloudList.length - 1].ID + "></img>"); // Append une image cloud1 ou cloud2 au body
                $nuage = $("#" + cloudList[cloudList.length - 1].ID);
                $nuage.css({
                    "display": "float",
                    "position": "fixed",
                    "height": "10%",
                    ...
                    "top": cloudList[cloudList.length - 1].Top
                });
            }
            cloudInterval();
        }, 8000)
    ...
    //Fin des fonctions spécifiques à la version Mario
    ...
    randomCloud();
    placePlayingBar();
    //relance la musique si elle se termine
    ...
    $("#mute").click(function () {
        if (backAudio.paused === true) {
            backAudio.play();
            $(this).css("background-image", "url('sound_on.png')")
        } else {
            backAudio.pause();
            $(this).css("background-image", "url('sound_off.png')")
        }
    });
